...
def generate_config(deepseek_key, mimo_key, vision_key, anysearch_key,
                    ...
                    character_name, owner_qq, llm_url, llm_model, vision_url, vision_model,
...
def main():
    ...
    bot_qq = ask("机器人 QQ 号（登录 NapCat 的账号）", required=True)
    owner_qq = ask("主人 QQ 号（能执行指令的管理员）", required=True)
    ...
    env = generate_env(
        llm_key, "", vision_key, anysearch_key,
        onebot_token, gateway_token, qq_app_id, qq_secret, ports,
        knowledge_dir, owner_qq
    )
    env_path = HERMES_HOME / ".env"
    env_path.write_text(env, encoding="utf-8")
    print(f"    ✓ {env_path}")

    # SOUL.md — 由用户自己准备（参考 templates/SOUL-template.md）
    soul_dst = HERMES_HOME / "SOUL.md"
    if not soul_dst.exists():
        print(f"    ⚠ SOUL.md 未创建 — 请参考 templates/SOUL-template.md 自行准备")
    ...
    if onebot_src.exists():
        ...
        onebot_dst = napcat_cfg / f"onebot11_{bot_qq}.json"
    ...
    if napcat_src.exists():
        napcat_dst = napcat_cfg / f"napcat_{bot_qq}.json"

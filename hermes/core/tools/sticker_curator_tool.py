...
Storage layout:
    $SOYO_COLLECTION_ROOT  (default ~/Pictures/soyo_collected/)
        <emotion>/<YYYYMMDD_HHMMSS>_<short_hash>.<ext>
    $SOYO_STICKER_INDEX    (default $HERMES_HOME/soyo_sticker_collection.json)
...
COLLECTION_ROOT = Path(os.getenv("SOYO_COLLECTION_ROOT",
                                  os.path.expanduser("~/Pictures/soyo_collected")))
INDEX_PATH = Path(os.getenv("SOYO_STICKER_INDEX",
                             str(_HERMES_HOME / "soyo_sticker_collection.json")))
MAX_PER_EMOTION = 20
MAX_TOTAL = 200
...
_ALLOWED_SOURCE_ROOTS = (
    _HERMES_HOME,  # cache/images/, state/, audio_cache/, ...
    Path("/tmp/"),
)

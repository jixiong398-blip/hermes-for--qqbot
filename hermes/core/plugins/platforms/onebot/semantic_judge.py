...
## 反面案例矫正（真实发生过的误判类型，避免重犯）

场景类型：群友A 先 @了群友B（例如约见面、送东西、聊一件具体的事），随后 A 连续发出**无 @ 的接续消息**（如"你在哪来着""我啥时候给你拿过去"这类围绕那件事的问句）。这些消息的真实指向是 **B**——A 在继续处理 A↔B 之间的事，与 $bot_name 无关。

曾发生误判：判定器看到"你在哪来着"这类无 @、无名字的问句，就推断"语境直接指向 $bot_name"并触发回复——实际 $bot_name 插入了 A↔B 的对话，是错误的。

正确判定：当 recent 消息中存在「某群友 @ 了另一位群友 + 话题围绕这两位」的明确线索时：
- 该群友之后的**无 @ 消息，默认指向被 @ 的那位群友**，而非 $bot_name
- 无主问句（"你在哪""什么时候给你""拿过去"等）优先归属到**最近的 @ 对象 / 话题对象**
- 仅当消息明确提到 $bot_name、回复 $bot_name 的消息、或直呼 $bot_name 名字时，才视为指向 $bot_name

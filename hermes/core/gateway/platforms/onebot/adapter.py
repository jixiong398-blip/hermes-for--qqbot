...
class OneBotAdapter(BasePlatformAdapter):
    ...
    def _persist_chat_message(self, group_id: str, chat_type: str, user_id: int,
        ...
        try:
            ...
            gw._store.add_chat_buffer(
                chat_id=str(user_id) if chat_type == "private" else group_id,
                chat_type=chat_type,
    ...
    async def connect(self) -> bool:
        ...
        self._load_config()
        logger.info("[OneBot] Connecting to WS %s, HTTP %s", self._ws_url, self._http_url)
        ...
        try:
            ...
            # Auto-recover recent messages while we were offline
    ...
    async def disconnect(self) -> None:
        ...
        if self._http_client:
            try:
                ...
                pass
            self._http_client = None
        for future in self._pending_echo.values():
            if not future.done():
                future.cancel()
        self._pending_echo.clear()

        # Cancel pending image batch tasks
        for task in self._pending_image_tasks.values():
            if not task.done():
                task.cancel()
        self._pending_image_tasks.clear()
        self._pending_image_events.clear()
    ...
    async def _flush_mention_batch(self, key: str, group_id):
        ...
        last = entries[-1]
        msg = dict(last["msg"])
        _self_qq = str(getattr(self, "_self_id", "") or "")
    ...
    async def _process_message(self, msg: dict) -> None:
        ...
        if msg_type == "group" and effective_self_id:
            ...
            if not is_mentioned and not starts_with_hash:
                logger.info("[OneBot] Group message without @mention or # prefix, lurking")
                return
        ...
        if reply_msg_id:
            ...
            try:
                from agent.memory.gateway import UnifiedMemoryGateway
                gw = UnifiedMemoryGateway.get_instance()
                db_row = gw._store.get_message_by_id(reply_msg_id)
                if db_row:
                    reply_raw = {
                        "raw_message": db_row["text"],
                        "sender": {"nickname": db_row["sender"]},
                    }
                    # Check if buffer text had [image:path] hints for vision
                    _db_img_matches = re.findall(r'\[image:([^\]]+)\]', db_row["text"])
                    if _db_img_matches:
                        for _m in _db_img_matches:
                            for _p in _m.split(","):
                                _p = _p.strip()
                                if _p and _p not in ("download_failed", "") and \
                                   not _p.startswith("qq_face:") and os.path.isfile(_p):
                                    reply_media_urls.append(_p)
                                    reply_media_types.append("image/jpeg")
                    logger.info("[OneBot] Reply context from DB: %s", db_row["text"][:80])
            except Exception:
                pass
    ...
    async def send(
        ...
        """Send a text message to a QQ chat. Multi-paragraph content is split into separate messages."""
        # Ensure multi-paragraph replies are always sent line-by-line
        if content and "\n" in content:
        ...
        return await self._send_text_with_retry(chat_id, content, max_retries=3)

    async def _send_text_with_retry(self, chat_id, content, max_retries=3):
        """Send text with automatic retry on failure."""
        logger.info("[OneBot] send() called: chat_id=%s, content_len=%d", chat_id, len(content) if content else 0)
        if not self._ws:

...
class AIAgent:
    ...
    def __init__(
        ...
        if self.api_mode == "anthropic_messages":
            ...
            _init_t0 = _init_time.perf_counter()
            logger.info("[PERF] AIAgent.__init__: client_kwargs resolved")
            ...
            try:
                ...
                logger.info("[PERF] AIAgent.__init__: client created %.3fs (init total so far)",
                            _init_time.perf_counter() - _init_t0)
                if not self.quiet_mode:
        ...
        try:
            logger.info("[PERF] __init__: tools loaded (%d) %.3fs", len(self.tools),
                        _init_time.perf_counter() - _init_t0)
        except NameError:
            pass
        ...
        try:
            logger.info("[PERF] __init__: memory/mgr done %.3fs",
                        _init_time.perf_counter() - _init_t0)
        except NameError:
            pass
        try:
        ...
        try:
            logger.info("[PERF] __init__: custom_providers done %.3fs",
                        _init_time.perf_counter() - _init_t0)
        except NameError:
            pass
        ...
        try:
            logger.info("[PERF] __init__: context engine done %.3fs",
                        _init_time.perf_counter() - _init_t0)
        except NameError:
            pass
        ...
        try:
            _feas_t0 = _init_time.perf_counter()
            self._check_compression_model_feasibility()
            logger.info("[PERF] __init__: compression feasibility %.3fs",
                        _init_time.perf_counter() - _feas_t0)
        except NameError:
            self._check_compression_model_feasibility()
        ...
        try:
            logger.info("[PERF] AIAgent.__init__: DONE %.3fs (total init)",
                        _init_time.perf_counter() - _init_t0)
        except NameError:
            pass
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        try:
            _sp_blocks = [
                (p.strip().split("\n")[0][:40], len(p.strip()) // 2)
                for p in prompt_parts if p.strip()
            ]
            _sp_blocks.sort(key=lambda x: -x[1])
            _sp_top = ", ".join(f"{n}~{t}t" for n, t in _sp_blocks[:6])
            logger.info("[PERF] system prompt built: %d chars (~%d tok). Top: %s",
                        len(_sp_final), len(_sp_final) // 2, _sp_top)
        except Exception:
            pass
        return _sp_final
    ...
    def run_conversation(
        ...
        _rc_t0 = _rc_time.perf_counter()
        logger.info("[PERF] run_conversation: start")

        self._ensure_db_session()
        logger.info("[PERF] run_conversation: _ensure_db_session %.3fs",
                    _rc_time.perf_counter() - _rc_t0)
        ...
        logger.info("[PERF] run_conversation: turn-logged %.3fs (pre-turn setup)",
                    _rc_time.perf_counter() - _rc_t0)
        ...
        if self._cached_system_prompt is None:
            ...
            if stored_prompt:
                ...
                logger.info("[PERF] run_conversation: system prompt REUSED %.3fs",
                            _rc_time.perf_counter() - _rc_t0)
            else:
                # First turn of a new session — build from scratch.
                logger.info("[PERF] run_conversation: building system prompt %.3fs",
                            _rc_time.perf_counter() - _rc_t0)
                self._cached_system_prompt = self._build_system_prompt(system_message)
                logger.info("[PERF] run_conversation: system prompt BUILT %.3fs",
                            _rc_time.perf_counter() - _rc_t0)

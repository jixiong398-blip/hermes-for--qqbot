...
# Decay constants — 1-day half-life tuned for fast-moving QQ group conversations
# where workflows decay quickly if not reinforced. With a 14-day half-life,
# stale procedural patterns persisted long past their usefulness; a 1-day
# half-life means a workflow drops to ~1% weight after ~6.6 idle days but
# still survives if used within the last 24h.
DECAY_HALF_LIFE_DAYS = 1.0   # Weight halves every 1 day idle
DECAY_MIN_WEIGHT = 0.02      # Slightly higher floor — forgotten threshold
DECAY_MAX_WEIGHT = 2.0       # Maximum weight cap
BOOST_PER_USE = 0.35         # Stronger boost per use (compensates faster decay)
PENALTY_PER_FAILURE = 0.15   # Stronger penalty per failed use

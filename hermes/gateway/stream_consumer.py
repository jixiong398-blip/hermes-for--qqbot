...
class GatewayStreamConsumer:
    ...
    def __init__(
        ...
        self._reply_anchor: Optional[str] = reply_to_id
    ...
    def on_delta(self, text: str) -> None:
        ...
        """
        if text:
            logger.debug("[StreamConsumer] on_delta: text_len=%d, text_preview=%r", len(text), text[:50])
            self._queue.put(text)
        elif text is None:
            logger.debug("[StreamConsumer] on_delta: segment break (None)")
    ...
    async def run(self) -> None:
        ...
        logger.info("[StreamConsumer] Started for chat_id=%s, adapter=%s", self.chat_id, type(self.adapter).__name__)
        ...
        try:
            while True:
                ...
                logger.debug("[StreamConsumer] Loop: got_done=%s, got_segment_break=%s, commentary=%s, accumulated_len=%d",
                             got_done, got_segment_break, commentary_text is not None, len(self._accumulated))
                ...
                if got_done:
                    ...
                    logger.info("[StreamConsumer] Stream DONE, accumulated_len=%d, already_sent=%s",
                                len(self._accumulated), self._already_sent)
                ...
                logger.debug("[StreamConsumer] should_edit=%s, elapsed=%.2f, interval=%.2f, accumulated_len=%d",
                             should_edit, elapsed, self._current_edit_interval, len(self._accumulated))
    ...
    async def _send_new_chunk(self, text: str, reply_to_id: Optional[str]) -> Optional[str]:
        ...
        On first send (reply_to_id is None), uses self._reply_anchor to quote
        the original triggering message.
        """
        ...
        try:
            ...
            # Use reply anchor for first send, or keep the existing message_id for edits
            effective_reply = reply_to_id or getattr(self, '_reply_anchor', None)
    ...
    async def _send_or_edit(self, text: str, *, finalize: bool = False) -> bool:
        ...
        if (self._message_id is None
            ...
            logger.debug("[StreamConsumer] Skipping short new msg: visible_len=%d", len(_visible_stripped))
            return True  # too short for a standalone message — accumulate more
        try:
            if self._message_id is not None:
                logger.debug("[StreamConsumer] Editing message %s, text_len=%d", self._message_id, len(text))
                if self._edit_supported:
                    ...
                    logger.info("[StreamConsumer] Edit result: success=%s, error=%s", result.success, result.error)
                    if result.success:
                        self._already_sent = True
                        self._last_sent_text = text
                ...
                logger.info("[StreamConsumer] Sending first message, text_len=%d, has_cursor=%s", len(text), self.cfg.cursor is not None and self.cfg.cursor in text)
                result = await self.adapter.send(
                    chat_id=self.chat_id,
                    content=text,
                    metadata=self.metadata,
                )
                logger.info("[StreamConsumer] Send result: success=%s, message_id=%s, error=%s", result.success, result.message_id, result.error)
                if result.success:

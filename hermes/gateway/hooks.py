...
class HookRegistry:
    ...
    def _register_builtin_hooks(self) -> None:
        ...
        try:
            from gateway.builtin_hooks.memory_maintenance import handle as mem_handle

            self._handlers.setdefault("agent:start", []).append(mem_handle)
            self._handlers.setdefault("agent:end", []).append(mem_handle)
            self._handlers.setdefault("session:end", []).append(mem_handle)
            self._handlers.setdefault("gateway:startup", []).append(mem_handle)
            self._loaded_hooks.append({
                "name": "memory-maintenance",
                "description": "Record turns + STM→LTM consolidation + periodic cleanup",
                "events": ["agent:start", "agent:end", "session:end", "gateway:startup"],
                "path": "(builtin)",
            })
        except Exception as e:
            print(f"[hooks] Could not load built-in memory-maintenance hook: {e}", flush=True)

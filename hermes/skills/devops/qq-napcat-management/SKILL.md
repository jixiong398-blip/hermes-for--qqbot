...
- Config (OneBot网络): `/home/ji/.napcat/config/onebot11_3560998016.json`
- Config (NapCat内核/防检测): `/home/ji/Napcat/opt/QQ/resources/app/app_launcher/napcat/config/napcat_3560998016.json`
- Log: `/tmp/napcat.log`
...
```bash
cd ~/Napcat && /home/ji/Napcat/opt/QQ/qq --no-sandbox -q 3560998016 &>/tmp/napcat.log &
```
...
cd ~/Napcat && /bin/xvfb-run -a --server-args="-screen 0 800x600x24" /home/ji/Napcat/opt/QQ/qq --no-sandbox -q 3560998016 &>/tmp/napcat.log &
```
...
```
Typical paths: `/tmp/napcat.log` (early init) or `/home/ji/Napcat/log/napcat_3560998016.log` (runtime).
...
`/home/ji/Napcat/opt/QQ/resources/app/app_launcher/napcat/config/napcat_3560998016.json`
...
**注意**：NapCat 自身还有一个全局配置 `/home/ji/Napcat/opt/QQ/resources/app/app_launcher/napcat/config/napcat.json`，里面的 bypass 默认都是 `false` — 这个文件是兜底默认值，实际生效的是带 QQ 号的配置文件（`napcat_3560998016.json`）。修改时要改带 QQ 号的那个。
...
1. **检查 watchdog 日志**：看 `napcat_3560998016.log` 中的实际错误信息
   ```bash
   grep -i "kicked\|KickedOffLine\|踢下线" /home/ji/Napcat/log/napcat_3560998016.log
   ```
2. **确认 bypass 全开**：检查 `napcat_3560998016.json` 中 bypass 是否全部 `true`
...
- ❗ The `uin` from cookie has an `o` prefix (e.g. `o3560998016`) — the script strips it automatically
...
```bash
env NAPCAT_QUICK_PASSWORD="your_qq_password" NAPCAT_QUICK_PASSWORD_MD5="" /bin/xvfb-run -a --server-args="..." /home/ji/Napcat/opt/QQ/qq --no-sandbox -q 3560998016
```
...
2. **日志关键词检测**: Scans `napcat_3560998016.log` 从上次检查位置往后读，检测两类告警：
...
python3 -c "
import os, json
total = int(os.popen('wc -l < /home/ji/Napcat/log/napcat_3560998016.log').read().strip())
state = json.load(open('/home/ji/.hermes/qq_watchdog_state.json'))
...
- ❗ On Linux, NapCat may create TWO log files: `/tmp/napcat.log` (old init) and `/home/ji/Napcat/log/napcat_3560998016.log` (actual runtime log). Always check the latter for real-time status. Use `readlink /proc/<pid>/fd/1` to discover the active log path.
...
- ❗ **NapCat 有两个 config 文件**：`napcat.json`（全局默认，bypass 全 false）和 `napcat_3560998016.json`（QQ 号专属，实际生效）。修改 bypass 要改带 QQ 号的那个，改全局默认不生效。

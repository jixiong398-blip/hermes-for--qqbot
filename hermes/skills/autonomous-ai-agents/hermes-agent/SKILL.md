...
The SQLite memory store (`~/.hermes/memory_store.db`) has a STM→LTM consolidation pipeline that is **fully implemented but currently has no automatic trigger**. The `memory_gateway` tool's `consolidate` action is also a stub. See `references/memory-consolidation-architecture.md` for full architecture, the gap, and fix options.
...
### Self-Improvement Review (background memory/skill review)

Automatic background process that reviews completed conversation turns for successful tool actions and surfaces a compact summary to the user.

- **What it does:** after each `run_conversation()` turn, Hermes checks whether `_memory_nudge_interval` (default: 10 turns) has elapsed. If so, it forks a lightweight `AIAgent` (max_iterations=16, quiet_mode=True, toolsets=["memory", "skills"]) in a background thread to review the conversation from scratch. The forked agent scans for successful `memory` or `skill_manage` tool calls and summarizes them.
- **Output format:** `💾 Self-improvement review: {action1} · {action2}` — e.g. `💾 Self-improvement review: Memory updated` or `💾 Self-improvement review: User profile updated`. The summary is emitted via `_safe_print()` (CLI) or `background_review_callback` (gateway/TUI).
- **Why it exists:** Catches memory/skill saves that happened mid-turn and makes them visible to the user without interrupting the main conversation. Prevents the "I don't remember anything" class of user complaints by showing that the agent is actively storing information.
- **How to configure:** `config.yaml → memory.nudge_interval` controls how often the review fires (default 10 turns). Set to 0 to disable. The review agent's `_memory_write_origin` is `"background_review"` — the `skill_provenance.py` provenance tracker uses this flag to distinguish agent-created skills from user-triggered ones.
- **User-visible effects:** Users may see `💾 Self-improvement review: …` messages after a turn where the main agent called `memory` or `skill_manage`. This is NOT an error — it's the background fork reporting what the foreground agent already did. If the message is distracting, increase `nudge_interval` or set it to 0.

Implementation: `run_agent.py` → `_spawn_background_review()` (line ~4117), `_summarize_background_review_actions()` (line ~4055). Trigger logic at line ~15145. Tests in `tests/run_agent/test_background_review*.py`.
...
- **Feishu reaction customization**: The processing-status reaction (⌨️ typing indicator) is configurable via `_FEISHU_REACTION_IN_PROGRESS` in `gateway/platforms/feishu.py`. Must modify BOTH the site-packages runtime file AND the source mirror. See `references/feishu-platform-customization.md` for supported values and the two-file edit pattern.

...
- 执行层输出契约：进入执行层的 mention/judge/continuation/exit 轮必须产出可见文本；执行层不提供沉默否决菜单，纯空输出或控制标记走同会话反馈重试（最多一次）
- exiting 单向棘轮禁止：bot 的“你们聊/你们继续”等客套措辞不得单独把 episode 推入 exiting；新的 @、角色别名直呼或回复 bot 必须即时复位退出状态

...
### Dashboard 与 NapCat 启动体验

- 重构 Dashboard 运维界面，保留服务控制、NapCat 账号、日志、记忆、会话、知识库和 Live2D 操作，并改善桌面与移动端布局。
- 修复 NapCat 启动时无效工作目录导致的 Windows `WinError 267`，增加安装目录和启动脚本预检。
- 增加普通 QQ 进程冲突检测、NapCat 启动状态轮询、OneBot/WebUI 端口就绪检查和结构化故障提示。
- 将 Dashboard 首次配置页加入升级映射，确保旧用户升级时页面资源完整。

### 验证

- Dashboard NapCat 预检测试通过。
- Windows 与 WSL 核心编译验证通过。
- 浏览器验证通过：桌面端和 390px 移动端无横向溢出，启动错误面板可显示可操作提示。

## v0.14.15 (2026-09-05)

### 可靠性与多平台兼容

- 修复 OneBot 完成回执与后续消息并发到达时的重复契约重试问题，避免额外 agent 轮次和潜在重复回复。
- 修复 Feishu WebSocket 运行时参数覆盖对全局 `websockets` 模块的污染，支持 Feishu 与 OneBot 并行使用。
- 增加 OneBot 沉默标记、空完成回执、发送回执和退出状态相关回归测试。
- 强化记忆、SessionDB、环境工具和 turn 生命周期的边界契约，减少跨聊天范围污染和异常完成写入。

### 配置与文档

- 修复知识库工具中的本机绝对路径，改为复用 `OBSIDIAN_VAULT_PATH`/当前 vault 配置。
- 新增脱敏的 QQ 空间可选配图指南，明确图片来源、白名单、回执、去重和 dry-run 规则。
- 更新生产验证交接文档和升级计划；运行配置、数据库、日志、sessions 与凭据继续由部署环境管理。
...
**群聊执行契约**（服务器 review 落地）：
- 群聊回复状态更新改为等待真实发送结果（Future 注册表 + 45s 兜底），避免回复文本/状态在 agent 实际完成前提前更新
- judge 模式新增独立分支（不再落入旁观模式措辞），附带判定理由

**看图时序修复**：
- 发图后紧跟提问时 bot 看不到图 → 等待窗口内所有待处理图片下载完成（单任务 30s 上限，最多 5 张）
- 图片下载失败显式提示（原静默删标记导致 bot 误判"没发图"）

**judge 反面案例矫正**：
- 群友 A @ 群友 B 后，A 的无 @ 接续消息正确归属 B 而非 bot，避免插话误判

**记忆与成长决策**（架构规划存档）：
- 记忆作用域 = 全局共享：存储层不分用户/群桶（单角色多群人设，跨群记性好是特性）；真正的边界在展示层（检索进 prompt 时不该展现的内容不外显）
- 身份成长 = 自传记忆：不加自动人格修订（SOUL 人格由用户掌控，改 SOUL 即回滚）；成长机制 = 自传体记忆（不衰减全量加载）

**仓库清理**：移除 hermes/scripts 软链跟踪（服务器部署时态，避免泄露服务器路径）

**涉及文件**：`adapter.py`、`group_executor.py`、`media_pipeline.py`、`semantic_judge.py`
